...
def apply_filters_dynamic(
    ...
    try:
        ...
        for row in cursor.fetchall():
            ...
            if last_analysis:
                try:
                    # Parse datetime, handle both aware and naive
                    analysis_dt = datetime.fromisoformat(last_analysis)
                    now = datetime.now(analysis_dt.tzinfo) if analysis_dt.tzinfo else datetime.now()
                    days_since = (now - analysis_dt).days
