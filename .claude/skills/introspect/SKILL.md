...
2. Extract only durable learnings.
3. Classify them with `keep/remove/fix/improve`.
4. Attach concrete evidence to each item.
5. Assign one owner to each item.
6. If the safe owner is obvious, apply the change now instead of leaving a note.
7. Keep the closeout compact.
