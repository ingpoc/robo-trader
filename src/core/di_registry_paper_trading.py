...
async def register_paper_trading_services(container: "DependencyContainer") -> None:
    ...
    async def create_paper_trading_price_monitor():
        from src.services.paper_trading.price_monitor import \
        ...
        price_monitor = PaperTradingPriceMonitor(
            event_bus, store, broadcast_coordinator
        )
        await price_monitor.initialize()
        logger.info(
            "PaperTradingPriceMonitor initialized - real-time WebSocket updates enabled"
        )
        return price_monitor

    container._register_singleton(
        "paper_trading_price_monitor", create_paper_trading_price_monitor
    )

    # Paper Trading Account Manager
    async def create_paper_trading_account_manager():
        from src.services.paper_trading.account_manager import \
        ...
        logger.info(
            "PaperTradingAccountManager created with Zerodha MarketDataService and PriceMonitor"
        )
        return manager

    container._register_singleton(
        "paper_trading_account_manager", create_paper_trading_account_manager
    )

    # Paper Trade Executor
    async def create_paper_trade_executor():
        from src.services.paper_trading.trade_executor import \
            PaperTradeExecutor
        ...
        logger.info(
            "PaperTradeExecutor created with MarketDataService for Phase 3 real-time execution prices"
        )
        return executor
    ...
    async def create_paper_trading_execution_service():
        from src.services.paper_trading_execution_service import \
    ...
    container._register_singleton(
        "paper_trading_execution_service", create_paper_trading_execution_service
    )
