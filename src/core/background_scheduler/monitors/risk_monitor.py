...
class RiskMonitor:
    ...
    async def check_stop_loss(
        self, portfolio: Dict[str, Any], stop_loss_percent: float = 5.0
    ) -> Dict[str, Any]:
        ...
        """
        result = {"breaches": [], "alerts_created": 0, "positions_checked": 0}
        ...
        for holding in portfolio.get("holdings", []):
            try:
                ...
                if stop_loss_breached and pnl_percent < 0:
                    ...
                    logger.warning(
                        f"Stop loss breached for {symbol}: {last_price:.2f} <= {stop_loss_price:.2f}"
                    )
        ...
        if result["alerts_created"] > 0:
            logger.info(
                f"Risk monitor: created {result['alerts_created']} stop loss alerts"
            )
        ...
        try:
            ...
            return {
                ...
                "risk_level": (
                    "high"
                    if pnl_percent < -10
                    else "medium" if pnl_percent < -5 else "low"
                ),
                "break_even_price": avg_price,
                "distance_from_breakeven": (
                    ((last_price - avg_price) / avg_price * 100) if avg_price else 0
                ),
            }
        except Exception as e:
    ...
    def calculate_portfolio_risk(portfolio: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for holding in holdings:
                try:
                    pnl_value = float(holding.get("pnl", 0))
                    total_value += float(holding.get("average_price", 0)) * float(
            ...
            portfolio_loss_pct = (
                (total_loss / total_value * 100) if total_value > 0 else 0
            )
            ...
            return {
                ...
                "risk_level": (
                    ...
                    else "medium" if portfolio_loss_pct > 2 else "low"
                ),
            }
        except Exception as e:
    ...
    async def check_concentration_risk(
        self, portfolio: Dict[str, Any], max_position_percent: float = 15.0
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            total_value = 0
            for holding in holdings:
                position_value = float(holding.get("average_price", 0)) * float(
                    holding.get("quantity", 0)
                )
                    ...
                    result["high_concentration_positions"].append(
                        {
                            ...
                            "concentration_level": (
                                "very_high" if position_percent > 30 else "high"
                            ),
                        }
                    )

                result["max_position_size"] = max(
                    result["max_position_size"], position_percent
                )
