...
class StatusBroadcastCoordinator(BaseCoordinator):
    ...
    async def broadcast_system_health(
        self, components: Dict[str, Any], timestamp: str, force: bool = False
    ) -> bool:
        ...
        try:
            ...
            self._broadcast_metrics["last_broadcast_time"] = datetime.now(
                timezone.utc
            ).isoformat()
            ...
            await self._broadcast_coordinator.broadcast_system_health_update(
                health_data
            )
