...
from .aggregation.status_aggregation_coordinator import \
...
class StatusCoordinator(BaseCoordinator):
    ...
    def __init__(
        ...
        self.broadcast_coordinator_internal = StatusBroadcastCoordinator(
            config, broadcast_coordinator
        )
    ...
    async def get_system_status(self, force_broadcast: bool = False) -> Dict[str, Any]:
        ...
        ai_status, scheduler_status_data, claude_status_data, portfolio_status = (
        ...
        components = await self.aggregation_coordinator.aggregate_system_components(
            (
                ...
                if not isinstance(scheduler_status_data, Exception)
                else {}
            ),
        ...
        status_data = {
            ...
            "claude_status": (
                claude_status_data
                if not isinstance(claude_status_data, Exception)
                else {}
            ),
            "portfolio_status": (
                portfolio_status if not isinstance(portfolio_status, Exception) else {}
            ),
