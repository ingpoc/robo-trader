...
class AIStatusCoordinator(BaseCoordinator):
    ...
    async def get_claude_agent_status(self) -> Dict[str, Any]:
        ...
        if claude_status and claude_status.is_valid:
            ...
            cli_process_running = claude_status.account_info.get(
                "cli_process_running", False
            )
            if sdk_connected and cli_process_running:
                return {
                    "status": "active",
                    "authMethod": claude_status.account_info.get(
                        "auth_method", "unknown"
                    ),
                    "tasksCompleted": claude_status.account_info.get(
                        "tasks_completed", 0
                    ),
                    "lastActivity": claude_status.account_info.get(
    ...
    def get_ai_analysis_status(self) -> str:
        ...
        try:
            from ...services.portfolio_intelligence_analyzer import \
