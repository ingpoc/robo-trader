...
from ...sdk_helpers import (query_only_with_timeout, query_with_timeout,
...
class QueryProcessingCoordinator(BaseCoordinator):
    ...
    async def process_query_enhanced(self, client, query: str) -> Dict[str, Any]:
        ...
        try:
            if not client:
                self._log_warning(
                    "Claude client not available - cannot process enhanced query"
                )
                return {
                    "thinking": [],
                    "tool_uses": [],
                ...
                if isinstance(message, AssistantMessage):
                    for block in message.content:
                        if isinstance(block, TextBlock):
                            ...
                            self._log_info(
                                f"Tool Result: {block.tool_use_id} - {status}"
                            )
