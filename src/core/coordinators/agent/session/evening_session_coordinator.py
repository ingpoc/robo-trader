...
Focused coordinator for evening review sessions.
...
from src.models.claude_agent import (ClaudeSessionResult, SessionType,
...
class EveningSessionCoordinator(BaseCoordinator):
    """
    Coordinates evening review sessions.

    Responsibilities:
    - Run evening review sessions
    - Handle evening session state
    - Process evening tool calls
    """
    ...
    async def run_evening_review_session(
        self, account_type: str, context: Dict[str, Any]
    ) -> ClaudeSessionResult:
        """Execute evening review session using Claude SDK."""
        ...
        try:
            prompt = (
                self._prompt_builder.build_evening_prompt(account_type, context)
            ...
            async for response in receive_response_with_timeout(
                self.client, timeout=180.0
            ):
                    ...
                    for tool_call in response.tool_calls:
                        tool_call_record = ToolCall(
                            tool_name=ToolCallType(
                                tool_call.name.replace("mcp__trading__", "")
                            ),
            ...
            await self.event_bus.publish(
                Event(
                    ...
                    data={
                        "session_id": session_id,
                        "session_type": SessionType.EVENING_REVIEW.value,
                    },
            ...
            self._log_error(f"Claude SDK error in evening session: {e}", exc_info=True)
            raise TradingError(
                f"Evening session failed: {e}",
                category=ErrorCategory.INTEGRATION,
                severity=ErrorSeverity.HIGH,
                recoverable=True,
            )
        except Exception as e:
            self._log_error(f"Error in evening session: {e}", exc_info=True)
            raise TradingError(
                f"Evening session failed: {e}",
                category=ErrorCategory.SYSTEM,
                severity=ErrorSeverity.HIGH,
