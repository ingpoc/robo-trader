...
class MorningSessionCoordinator(BaseCoordinator):
    ...
    async def run_morning_prep_session(
        ...
        try:
            prompt = (
                self._prompt_builder.build_morning_prompt(account_type, context)
            ...
            async for response in receive_response_with_timeout(
                ...
                claude_responses, tokens = self._process_response(
                    response, tool_calls_history
                )
                if claude_responses:
            ...
            await self.event_bus.publish(
                Event(
                    ...
                    data={
                        "session_id": session_id,
                        "session_type": SessionType.MORNING_PREP.value,
                    },
