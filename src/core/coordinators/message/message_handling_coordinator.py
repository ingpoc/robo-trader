...
class MessageHandlingCoordinator(BaseCoordinator):
    ...
    async def register_default_handlers(self, routing_coordinator) -> None:
        ...
        await routing_coordinator.register_handler(
            MessageType.ANALYSIS_RESPONSE, handle_analysis_response
        )
        ...
        await routing_coordinator.register_handler(
            MessageType.DECISION_PROPOSAL, handle_decision_proposal
        )
        ...
        await routing_coordinator.register_handler(
            MessageType.STATUS_UPDATE, handle_status_update
        )

    async def _handle_analysis_response(self, message: AgentMessage) -> None:
        """Handle analysis response messages."""
        logger.debug(
            f"Analysis response from {message.sender_agent}: {len(str(message.content))} chars"
        )
