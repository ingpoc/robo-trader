...
class QueueExecutionCoordinator(BaseCoordinator):
    ...
    async def execute_queues_sequential(self) -> Dict[str, Any]:
        ...
        try:
            if self._sequential_queue_manager:
                ...
                self._log_info(
                    "Sequential queue execution completed through SequentialQueueManager"
                )
                return {
                    "success": True,
                ...
                self._log_warning(
                    "SequentialQueueManager not available, using fallback execution"
                )
                results = {}
                execution_order = [
    ...
    async def execute_queues_concurrent(
        ...
        self._log_info(
            f"Starting concurrent queue execution (max_concurrent={max_concurrent})"
        )
