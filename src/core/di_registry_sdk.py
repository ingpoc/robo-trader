...
async def register_sdk_services(container: "DependencyContainer") -> None:
    ...
    async def create_tool_executor():
        ...
        risk_config = (
            container.config.risk.__dict__ if hasattr(container.config, "risk") else {}
        )
    ...
    async def create_response_validator():
        from src.services.claude_agent.response_validator import \
            ResponseValidator
    ...
    container._register_singleton(
        "claude_sdk_client_manager", create_claude_sdk_client_manager
    )
    ...
    container._register_singleton(
        "claude_agent_mcp_server", create_claude_agent_mcp_server
    )
    ...
    async def create_execution_monitor():
        from src.services.claude_agent.execution_monitor import \
            ExecutionMonitor
    ...
    async def create_daily_strategy_evaluator():
        from src.services.claude_agent.daily_strategy_evaluator import \
    ...
    container._register_singleton(
        "daily_strategy_evaluator", create_daily_strategy_evaluator
    )

    # Activity Summarizer Service
    async def create_activity_summarizer():
        from src.services.claude_agent.activity_summarizer import \
            ActivitySummarizer
    ...
    async def create_trade_decision_logger():
        from src.services.claude_agent.trade_decision_logger import \
            TradeDecisionLogger
    ...
    async def create_prompt_optimization_tools():
        from src.services.claude_agent.prompt_optimization_tools import \
    ...
    container._register_singleton(
        "prompt_optimization_tools", create_prompt_optimization_tools
    )
