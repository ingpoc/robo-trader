        ...
        async with self._lock:
            ...
            """,
                (
                    ...
                    report_date,
                    kwargs.get("eps_actual"),
                    kwargs.get("eps_estimated"),
                    kwargs.get("revenue_actual"),
                    kwargs.get("revenue_estimated"),
                    kwargs.get("surprise_pct"),
                    kwargs.get("guidance"),
                    kwargs.get("next_earnings_date"),
                    now,
                    now,
                ),
            ):
                        ...
                        try:
                            ...
                            if "T" in next_earnings_date_str:
                                next_date = datetime.fromisoformat(
                                    next_earnings_date_str.replace("Z", "+00:00")
                                ).date()
                            else:
                                next_date = datetime.fromisoformat(
                                    next_earnings_date_str
                                ).date()
                            ...
                            logger.debug(
                                f"Error parsing next_earnings_date {next_earnings_date_str}: {e}"
                            )
                            continue
