...
from ..models.claude_agent import (ClaudeSessionResult, SessionType,
                                   StrategyLearning)
...
class ClaudeStrategyStore:
    ...
    async def save_session(self, session: ClaudeSessionResult) -> None:
        ...
        learnings_json = json.dumps(
            session.learnings.to_dict() if session.learnings else None
        )
        what_worked_json = json.dumps(
            session.learnings.what_worked if session.learnings else []
        )
        what_failed_json = json.dumps(
            session.learnings.what_failed if session.learnings else []
        )
        ...
        logger.info(
            f"Saved session: {session.session_id} ({session.session_type.value})"
        )
    ...
    async def get_session(self, session_id: str) -> Optional[ClaudeSessionResult]:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            cursor = await db.execute(
                "SELECT * FROM claude_strategy_logs WHERE session_id = ?", (session_id,)
            )
    ...
    async def save_token_usage(
        ...
        logger.info(
            f"Logged token usage: {operation} ({input_tokens}in, {output_tokens}out)"
        )
    ...
    async def get_sessions_count(self, account_type: Optional[str] = None) -> int:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            if account_type:
                ...
                cursor = await db.execute("SELECT COUNT(*) FROM claude_strategy_logs")
            row = await cursor.fetchone()
