...
from src.core.di import (cleanup_container,
                         initialize_container)
from src.core.errors import ErrorHandler, TradingError

from .claude_agent_api import router as claude_agent_router
from .connection_manager import ConnectionManager
from .queues_api import router as queues_router
# Import route modules
from .routes import (agents_router, analytics_router, dashboard_router,
                     execution_router, monitoring_router)
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        log_level = os.getenv("LOG_LEVEL", "INFO").upper()
        setup_logging(
            logs_dir, log_level, clear_logs=False
        )  # Don't clear again, already cleared
        ...
        class LoguruHandler(std_logging.Handler):
            def emit(self, record):
                try:
                    ...
                    level = (
                        log_level  # Use configured level instead of defaulting to INFO
                    )
    ...
    logger.info(
        "Container stored in app.state - all routes can access via Depends(get_container)"
    )
    ...
    logger.info(
        "Initializing orchestrator (starting background scheduler and queue manager)..."
    )
    await orchestrator.initialize()
    logger.info(
        "Orchestrator initialization complete - BackgroundScheduler and SequentialQueueManager are now running"
    )
    ...
    try:
        ...
        if queue_coordinator:
            ...
            logger.info(
                "Queue execution started - queues are now processing pending tasks"
            )
        else:
            logger.warning("Queue coordinator not available - queues will not start")
    except Exception as e:
        logger.warning(
            f"Failed to start queues: {e} - continuing without queue execution"
        )
...
async def logging_middleware(request: Request, call_next):
    ...
    try:
        ...
        if status_code >= 500:
            logger.error(
                f"{method} {path} - Status: {status_code} - Time: {process_time:.3f}s - Client: {client_ip}"
            )
        ...
        process_time = time.time() - start_time
        logger.error(
            f"{method} {path} - Exception: {type(e).__name__}: {str(e)} - Time: {process_time:.3f}s - Client: {client_ip}",
            exc_info=True,
        )
        raise
...
async def error_handling_middleware(request: Request, call_next):
    ...
    try:
        ...
        logger.error(
            f"Trading error: {e.context.message}",
            extra={
        ...
        return JSONResponse(
            status_code=(
                500 if e.context.severity.value in ["critical", "high"] else 400
            ),
            content=e.to_dict(),
        )
    except Exception as e:
        ...
        logger.error(
            f"Unhandled error: {error_context.message}",
            extra={
        ...
        return JSONResponse(
            status_code=500, content=ErrorHandler.format_error_response(e)
        )
...
async def get_ai_data_with_retry(
    orchestrator, connection_id: str, max_retries: int = 2
) -> tuple:
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            ai_status_task = asyncio.wait_for(
                orchestrator.get_ai_status(), timeout=timeout
            )
            ...
            if isinstance(ai_status_result, Exception):
                logger.warning(
                    f"AI status failed (attempt {attempt + 1}): {ai_status_result}"
                )
            ...
            if isinstance(recommendations_result, Exception):
                logger.warning(
                    f"Recommendations failed (attempt {attempt + 1}): {recommendations_result}"
                )
                recommendations = []
            else:
                recommendations = recommendations_result

            if not isinstance(ai_status_result, Exception) or not isinstance(
                recommendations_result, Exception
            ):
            ...
            if attempt == max_retries:
                ...
                if (
                    ...
                    or "open_process" in str(e)
                ):
                    ai_status = {
                        "status": "unavailable",
                        "error": "AI agent temporarily unavailable - paper trading mode active",
                    }
                else:
    ...
    try:
        ...
        if container:
            try:
                ...
                if orchestrator:
                    ...
                    logger.info(
                        f"Broadcast initial status updates to client {client_id}"
                    )
            except Exception as e:
                logger.warning(
                    f"Failed to broadcast initial status to client {client_id}: {e}"
                )
        ...
        if connection_manager:
            try:
                ...
                logger.warning(
                    f"Failed to disconnect WebSocket {client_id}: {disconnect_error}"
                )
