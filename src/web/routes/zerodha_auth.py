...
from src.web.utils.error_handlers import (handle_trading_error,
...
async def initiate_zerodha_auth(
    ...
    try:
        ...
        if token_data:
            logger.info(
                f"Valid OAuth token found for user: {token_data.get('user_id')}, token not expired"
            )
            return {
                "success": False,
                "auth_required": False,
...
async def zerodha_oauth_callback(
    ...
    state: Optional[str] = Query(
        None, description="State parameter for CSRF validation (optional)"
    ),
    ...
    try:
        ...
        try:
            ...
            if broker:
                ...
                if auth_success:
                    logger.info(
                        f"Broker client initialized successfully for user: {result.get('user_id')}"
                    )
                else:
            ...
            if orchestrator:
                ...
                logger.info(
                    f"Portfolio scan completed after OAuth: {scan_result.get('source', 'unknown')}"
                )
            else:
                logger.warning("Orchestrator not available for auto portfolio scan")
        except Exception as e:
            logger.error(
                f"Failed to auto-trigger portfolio scan after OAuth: {e}", exc_info=True
            )
...
async def logout_zerodha(
    ...
    try:
        ...
        return {"success": True, "message": "Successfully logged out from Zerodha"}
