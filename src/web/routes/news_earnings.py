...
async def get_news_earnings_general(
    request: Request,
    news_limit: int = 10,
    earnings_limit: int = 5,
    ...
    try:
        ...
        try:
            ...
            logger.debug(
                f"Could not get portfolio symbols for general news/earnings: {e}"
            )
        ...
        if portfolio_symbols:
            ...
            for symbol in portfolio_symbols[:10]:  # Limit to first 10 symbols
                symbol_news = await state_manager.get_news_for_symbol(
                    symbol, limit=news_limit
                )
        ...
        if portfolio_symbols:
            ...
            for symbol in portfolio_symbols[:10]:  # Limit to first 10 symbols
                symbol_earnings = await state_manager.get_earnings_for_symbol(
                    symbol, limit=earnings_limit
                )
...
async def get_news_earnings(
    ...
    try:
        ...
        symbol_earnings_raw = await state_manager.get_earnings_for_symbol(
            symbol, limit=earnings_limit
        )
            ...
            symbol_earnings.append(
                {
                    ...
                    "fetched_at": earnings.get(
                        "fetched_at", datetime.now(timezone.utc).isoformat()
                    ),
                    "created_at": earnings.get(
            ...
            formatted_news.append(
                {
                    ...
                    ),  # Database has "title", but check for "headline" for compatibility
                    "summary": news.get(
                        "summary", news.get("content", "")
                    ),  # Database has "summary" field, content is separate
                    "content": news.get(
...
async def get_upcoming_earnings(
    ...
    try:
        ...
        try:
            upcoming_earnings_raw = await state_manager.get_upcoming_earnings(
                days_ahead
            )
            logger.info(
                f"Retrieved {len(upcoming_earnings_raw)} upcoming earnings from database"
            )
        except Exception as e:
            logger.error(
                f"Failed to fetch upcoming earnings from database: {e}", exc_info=True
            )
            raise
        ...
        for earnings in upcoming_earnings_raw:
            try:
                    ...
                    try:
                        ...
                        if "T" in str(next_earnings_date_str):
                            next_date = datetime.fromisoformat(
                                str(next_earnings_date_str).replace("Z", "+00:00")
                            ).date()
                        else:
                            # Date only format (YYYY-MM-DD)
                            next_date = datetime.fromisoformat(
                                str(next_earnings_date_str)
                            ).date()
                        ...
                        logger.debug(
                            f"Could not parse next_earnings_date {next_earnings_date_str}: {e}"
                        )
                        pass
                ...
                upcoming_earnings.append(
                    {
                        ...
                        "company": earnings.get(
                            "company", f"{earnings.get('symbol', '')} Limited"
                        ),
                        "date": next_earnings_date_str
                        ...
                        "surprise_percent": earnings.get("surprise_pct"),
                        "status": (
                            "upcoming" if not earnings.get("eps_actual") else "reported"
                        ),
                ...
                logger.warning(
                    f"Error processing earnings entry: {e}, skipping", exc_info=True
                )
                continue
            ...
            formatted_recommendations.append(
                {
                    ...
                    "thesis": getattr(
                        rec,
                        "reasoning",
                        "AI-generated recommendation based on fundamental and technical analysis",
                    ),
