...
async def get_research_transparency(
    ...
    try:
        ...
        if not research_tracker:
            return JSONResponse(
                {"error": "Research tracker not available"}, status_code=500
            )
...
@limiter.limit(transparency_limit)
async def get_analysis_transparency(
    request: Request, container: DependencyContainer = Depends(get_container)
) -> Dict[str, Any]:
    ...
    try:
            ...
            if not configuration_state:
                ...
                if config_data and "analyses" in config_data:
                    for analysis_record in config_data["analyses"]:
                        try:
                            analysis_data = (
                                json.loads(analysis_record["analysis"])
                                if isinstance(analysis_record["analysis"], str)
                                else analysis_record["analysis"]
                            )
                            ...
                            portfolio_analyses.append(
                                {
                                    ...
                                    "confidence_score": analysis_data.get(
                                    ...
                                    "data_quality": analysis_data.get(
                                        "data_quality", {}
                                    ),
                                }
                            )
                            ...
                            logger.warning(
                                f"Failed to parse analysis JSON for {analysis_record.get('symbol', 'unknown')}: {e}"
                            )
                            continue

        except Exception as e:
            logger.warning(
                f"Could not get portfolio analyses from configuration state: {e}"
            )
            portfolio_analyses = []
        ...
        portfolio_stats = {
            ...
            "symbols_analyzed": len(
                set(analysis["symbol"] for analysis in portfolio_analyses)
            ),
            "avg_confidence": (
                sum(analysis["confidence_score"] for analysis in portfolio_analyses)
                / len(portfolio_analyses)
                if portfolio_analyses
                else 0.0
            ),
            "total_recommendations": sum(
                analysis["recommendations_count"] for analysis in portfolio_analyses
            ),
        }
...
@limiter.limit(transparency_limit)
async def get_execution_transparency(
    request: Request, container: DependencyContainer = Depends(get_container)
) -> Dict[str, Any]:
    ...
    try:
        ...
        if not claude_agent_service:
            return JSONResponse(
                {"error": "Claude agent service not available"}, status_code=500
            )
        ...
        if not strategy_store:
            return JSONResponse(
                {"error": "Strategy store not available"}, status_code=500
            )

        # Get recent sessions
        recent_sessions = await strategy_store.get_recent_sessions(
            "swing", limit=10
        ...
        execution_data = {
            ...
            "total_trades_executed": sum(
                len(s.decisions_made) for s in recent_sessions if s.decisions_made
            ),
            "total_token_usage": sum(
                s.token_input + s.token_output for s in recent_sessions
            ),
            ...
            "recent_sessions": [
                {
                    ...
                    "trades_executed": (
                        len(session.decisions_made) if session.decisions_made else 0
                    ),
...
@limiter.limit(transparency_limit)
async def get_daily_evaluation_transparency(
    request: Request, container: DependencyContainer = Depends(get_container)
) -> Dict[str, Any]:
    ...
    try:
        ...
        evaluation_data = {
            ...
            "avg_confidence": sum(e["confidence_score"] for e in daily_evaluations)
            / len(daily_evaluations),
...
@limiter.limit(transparency_limit)
async def get_daily_summary_transparency(
    request: Request, container: DependencyContainer = Depends(get_container)
) -> Dict[str, Any]:
    ...
    try:
        ...
        if not research_tracker or not strategy_store:
            return JSONResponse(
                {"error": "Required services not available"}, status_code=500
            )
        ...
        summary_data = {
            ...
            "total_research_sessions": len(
                await research_tracker.get_research_history(limit=100)
            ),
            ...
            "total_cost_usd": sum(s.total_cost_usd for s in recent_sessions)
            + research_stats["total_cost_usd"],
            "avg_session_confidence": (
                sum(s.confidence_score for s in recent_sessions) / len(recent_sessions)
                if recent_sessions
                else 0
            ),
            "key_activities": [
...
async def get_trade_decisions(
    ...
    try:
        ...
        if not trade_decision_logger:
            return JSONResponse(
                {"error": "Trade decision logger not available"}, status_code=500
            )
...
async def get_current_strategy(
    ...
    try:
        ...
        recent_sessions = await strategy_store.get_recent_sessions(
            account_type, limit=1
        )

        if not recent_sessions:
            return {"strategy": None, "message": "No strategy available yet"}
        ...
        strategy = {
            ...
            "focus_areas": (
                session.metadata.get("focus_areas", [])
                if hasattr(session, "metadata") and session.metadata
                else []
            ),
            "risk_level": (
                session.metadata.get("risk_level", "moderate")
                if hasattr(session, "metadata") and session.metadata
                else "moderate"
            ),
            "current_analysis": (
                session.metadata.get("current_analysis", "")
                if hasattr(session, "metadata") and session.metadata
                else ""
            ),
            "data_quality": (
                session.metadata.get("data_quality_summary", {})
                if hasattr(session, "metadata") and session.metadata
                else {}
            ),
            "last_updated": (
                session.timestamp.isoformat()
                if hasattr(session, "timestamp")
        ...
        return {
            "strategy": strategy,
            "session_id": (
                session.session_id if hasattr(session, "session_id") else None
            ),
        }
