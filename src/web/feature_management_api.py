...
from ..core.errors import FeatureManagementError
from ..services.feature_management import (BulkFeatureUpdate,
                                           DependencyType, FeatureConfig,
                                           FeatureDependency,
                                           FeatureManagementService,
                                           FeatureMetadata, FeatureStatus, FeatureToggleRequest,
                                           FeatureType)
...
class FeatureMetadataRequest(BaseModel):
    ...
    feature_type: str = Field(
        ..., regex="^(agent|service|algorithm|monitor|ui_component|integration)$"
    )
...
class FeatureDependencyRequest(BaseModel):
    ...
    dependency_type: str = Field(
        ..., regex="^(requires|conflicts|enhances|deprecated_by)$"
    )
...
class FeatureConfigRequest(BaseModel):
    ...
    dependencies: List[FeatureDependencyRequest] = Field(
...
async def get_feature_service(request: Request) -> FeatureManagementService:
    ...
    if not container:
        raise HTTPException(
            status_code=500, detail="Dependency container not available"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Feature management service not available"
        )
...
async def list_features(
    ...
    try:
        ...
        for feature in features:
            state = next(
                (s for s in states if s.feature_id == feature.feature_id), None
            )
...
async def get_feature(
    ...
    try:
        ...
        validation_warnings = await feature_service.validate_feature_dependencies(
            feature_id
        )
...
async def update_feature(
    ...
    try:
        ...
        if feature_request.feature_id != feature_id:
            raise HTTPException(
                status_code=400, detail="Feature ID in URL and body must match"
            )
...
async def get_health(
    ...
    try:
        ...
        healthy_features = sum(
            1 for state in states if state.health_status == "healthy"
        )
        error_features = sum(
            1 for state in states if state.status == FeatureStatus.ERROR
        )
        ...
        health_status = {
            ...
            "health_percentage": (
                (healthy_features / total_features * 100) if total_features > 0 else 100
            ),
            "timestamp": datetime.now(timezone.utc).isoformat(),
        }
