        ...
        try:
            ...
            return {
                "content": [
                    {
                        "type": "text",
                        "text": f"Fundamental screening completed. Found {len(candidates)} candidates",
                    },
...
def _simulate_screening(config: Config) -> List[Dict[str, Any]]:
    ...
    for symbol in symbols:
        ...
        # Apply filters
        if (
            pe_ratio <= config.screening.max_pe_ratio
            and roe >= config.screening.min_roe_percent
            and debt_equity <= config.screening.max_debt_equity
            and market_cap >= config.screening.min_market_cap / 10000000
        ):  # Convert to crores
