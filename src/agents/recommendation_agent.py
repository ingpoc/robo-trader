...
def create_recommendation_tools(
    ...
    @tool(
        ...
        "Generate AI-powered investment recommendation for a stock symbol",
        ...
        try:
            ...
            if not result:
                return {
                    "content": [
                        {
                            "type": "text",
                            "text": f"Unable to generate recommendation for {symbol} - insufficient data",
                        }
                    ],
                    "is_error": True,
                }
            ...
            return {
                "content": [
                    {
                        "type": "text",
                        "text": f"Generated {result.recommendation_type} recommendation for {symbol}",
                    },
            ...
            logger.error(
                f"Recommendation generation failed for {args.get('symbol', 'unknown')}: {e}"
            )
            return {
                "content": [
                    {
    ...
    @tool(
        ...
        "Generate recommendations for multiple stock symbols",
        {"symbols": List[str], "force_refresh": bool},
    )
    async def generate_bulk_recommendations_tool(
        args: Dict[str, Any],
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            recommendations = await reco_engine.generate_bulk_recommendations(
            ...
            return {
                "content": [
                    {
                        "type": "text",
                        "text": f"Generated recommendations for {len(results)}/{len(symbols)} symbols",
                    },
            ...
            return {
                "content": [
                    {
                        "type": "text",
                        "text": f"Error generating bulk recommendations: {str(e)}",
                    }
                ],
                "is_error": True,
            }
    ...
    @tool(
        ...
        "Get historical recommendations for a stock symbol",
    ...
    async def get_recommendation_history_tool(args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            recommendations = await reco_engine.get_recommendation_history(
                symbol, limit
            )
            ...
            return {
                "content": [
                    {
                        "type": "text",
                        "text": f"Found {len(history)} historical recommendations for {symbol}",
                    },
            ...
            logger.error(
                f"Failed to get recommendation history for {args.get('symbol', 'unknown')}: {e}"
            )
            return {
                "content": [
                    {
                        "type": "text",
                        "text": f"Error getting recommendation history: {str(e)}",
                    }
                ],
                "is_error": True,
            }
    ...
    async def analyze_portfolio_recommendations_tool(
        args: Dict[str, Any],
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not portfolio or not portfolio.holdings:
                return {
                    "content": [
                        {
                            "type": "text",
                            "text": "No portfolio holdings found to analyze",
                        }
                    ],
                    "is_error": True,
                }
            ...
            symbols = [
                ...
                if holding.get("tradingsymbol")
            ]
            if not symbols:
                return {
                    "content": [
                        {"type": "text", "text": "No valid symbols found in portfolio"}
                    ],
                    "is_error": True,
                }
                        ...
                        analysis["high_confidence_opportunities"].append(
                            {
                                ...
                                "target_price": result.target_price,
                                "potential_upsite": (
                                    f"{((result.target_price - await reco_engine._get_current_price(symbol)) / await reco_engine._get_current_price(symbol) * 100):.1f}%"
            ...
            for result in recommendations.values():
                try:
                    ...
                    logger.error(
                        f"Error storing recommendation for {result.symbol}: {e}"
                    )
            ...
            return {
                "content": [
                    {
                        "type": "text",
                        "text": f"Portfolio analysis complete: {len(recommendations)} recommendations generated",
                    },
            ...
            return {
                "content": [
                    {
                        "type": "text",
                        "text": f"Error analyzing portfolio recommendations: {str(e)}",
                    }
                ],
                "is_error": True,
            }
    ...
    @tool(
        ...
        "Get recommendation engine statistics and performance",
        {},
    )
    async def get_recommendation_stats_tool(args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                all_recommendations = await state_manager.get_all_recommendations(
                    limit=1000
                )
                ...
                if all_recommendations:
                    ...
                    completed_recommendations = [
                        r for r in all_recommendations if r.outcome
                    ]

                    if completed_recommendations:
                        successful_trades = sum(
                            1
                            for r in completed_recommendations
                            if r.actual_return and r.actual_return > 0
                        )
                        success_rate = (
                            successful_trades / len(completed_recommendations) * 100
                        )
                        ...
                        avg_return = sum(
                            ...
                            [r for r in completed_recommendations if r.actual_return]
                        )

                        stats.update(
                            {
                                "total_completed_recommendations": len(
                                    completed_recommendations
                                ),
                                "success_rate_percent": round(success_rate, 2),
                                "average_return_percent": round(avg_return, 2),
                                "total_tracked_recommendations": len(
                                    all_recommendations
                                ),
                            }
                        )
                    else:
                        stats["total_tracked_recommendations"] = len(
                            all_recommendations
                        )
                        stats["note"] = (
                            "No completed recommendations with outcome data yet"
                        )
            ...
            return {
                "content": [
                    {
                        "type": "text",
                        "text": f"Error getting recommendation statistics: {str(e)}",
                    }
                ],
                "is_error": True,
            }
