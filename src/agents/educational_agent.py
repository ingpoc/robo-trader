...
def create_educational_tools(
    ...
    @tool(
        "explain_concept",
        "Explain a trading concept or indicator",
        {"concept": str, "context": str, "detail_level": str},
    )
    async def explain_concept_tool(args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            explanation = await _generate_concept_explanation(
                concept, context, detail_level
            )
            ...
            return {
                "content": [
                    {"type": "text", "text": f"Error explaining concept: {str(e)}"}
                ],
                "is_error": True,
            }
    ...
    @tool(
        ...
        {"intent_id": str, "decision_type": str},
    )
    async def explain_decision_tool(args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "content": [
                    {"type": "text", "text": f"Error explaining decision: {str(e)}"}
                ],
                "is_error": True,
            }
    ...
    @tool(
        "explain_portfolio",
        "Explain portfolio composition and strategy",
        {"focus_area": str, "user_experience": str},
    )
    async def explain_portfolio_tool(args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            explanation = await _generate_portfolio_explanation(
                portfolio, focus_area, user_experience
            )
            ...
            return {
                "content": [
                    {"type": "text", "text": f"Error explaining portfolio: {str(e)}"}
                ],
                "is_error": True,
            }
...
async def _generate_concept_explanation(
    ...
    concept_explanations = explanations.get(
        concept,
        {
    ...
    base_explanation = concept_explanations.get(
        detail_level, concept_explanations.get("intermediate", "")
    )
...
async def _generate_portfolio_explanation(
    portfolio: PortfolioState, focus_area: str, user_experience: str
) -> str:
...
"""
            ...
            explanation += (
                "Risk metrics will be calculated after running a portfolio scan."
            )
...
"""
        if (
            portfolio.risk_aggregates.portfolio
            and portfolio.risk_aggregates.portfolio.sector_exposure
        ):
            ...
            for sector, exposure in sectors.items():
                ...
                explanation += (
                    f"- **{sector.title()}:** {percentage:.1f}% (₹{exposure:.0f})\n"
                )
...
Asset allocation determines your portfolio's risk and return characteristics. Different sectors perform differently in various market conditions. A balanced allocation reduces risk while maintaining growth potential."""
            ...
            explanation += (
                "Sector allocation will be analyzed after running a portfolio scan."
            )
