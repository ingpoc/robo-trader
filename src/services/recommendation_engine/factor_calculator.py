...
class FactorCalculator:
    ...
    def __init__(
        self, fundamental_service: FundamentalService, risk_service: RiskService
    ):
    ...
    async def calculate_valuation_score(
        ...
        try:
            ...
            if (
                hasattr(fundamental_data, "debt_to_equity")
                and fundamental_data.debt_to_equity
            ):
    ...
    async def calculate_growth_score(
        ...
        try:
            ...
            if (
                hasattr(fundamental_data, "revenue_growth")
                and fundamental_data.revenue_growth is not None
            ):
            ...
            # EPS growth analysis
            if (
                hasattr(fundamental_data, "eps_growth")
                and fundamental_data.eps_growth is not None
            ):
            ...
            if (
                hasattr(fundamental_data, "profit_margin")
                and fundamental_data.profit_margin is not None
            ):
    ...
    async def calculate_all_factors(
        ...
        if not fundamental_data:
            fundamental_data = await self.fundamental_service.get_fundamental_analysis(
                symbol
            )

        fundamental_score = (
            getattr(fundamental_data, "fundamental_score", 65.0)
            if fundamental_data
            else 50.0
        )
