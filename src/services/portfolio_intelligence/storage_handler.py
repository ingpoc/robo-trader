...
class PortfolioStorageHandler:
        ...
        try:
            ...
            for symbol in stocks_data.keys():
                analysis_data = {
                    ...
                    "data_quality": {
                        "has_earnings": len(stocks_data[symbol].get("earnings", []))
                        > 0,
                        "has_news": len(stocks_data[symbol].get("news", [])) > 0,
                        "has_fundamentals": len(
                            stocks_data[symbol].get("fundamental_analysis", [])
                        )
                        > 0,
            ...
            logger.info(
                f"Stored analysis results for {len(stocks_data)} symbols in database"
            )
    ...
    async def store_recommendation(
        self, recommendation: Dict[str, Any], analysis_id: str
    ) -> None:
        ...
        try:
            ...
            if success:
                logger.debug(
                    f"Stored recommendation for {recommendation.get('symbol', 'UNKNOWN')}"
                )
            else:
                logger.warning(
                    f"Failed to store recommendation for {recommendation.get('symbol', 'UNKNOWN')}"
                )
