...
class ZerodhaOAuthService:
    ...
    def get_redirect_url(self) -> str:
        ...
        if environment not in self._redirect_urls:
            logger.warning(
                f"Unknown environment '{environment}', using development redirect URL"
            )
    ...
    async def handle_callback(
        self, request_token: str, state: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if state:
                if not await self._validate_state(state):
                    ...
                    logger.info(
                        f"State parameter validated successfully: {state[:10]}..."
                    )
            else:
                logger.warning(
                    "No state parameter received from Zerodha callback - continuing without CSRF validation"
                )
            ...
            logger.info(
                f"Successfully authenticated Zerodha user: {access_token_data.get('user_id')}"
            )
            ...
            await self._emit_oauth_event(
                "oauth_error",
                {
                    ...
                    "error": str(e),
                    "request_token": (
                        request_token[:10] + "..." if request_token else None
                    ),
                },
            )
    ...
    async def _exchange_request_token(self, request_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            api_secret = getattr(
                self.config.integration, "zerodha_api_secret", ""
            ).strip()
            ...
            def exchange_token():
                ...
                try:
                    ...
                    logger.info(
                        f"  API Key: {api_key[:10]}... (length: {len(api_key)})"
                    )
                    logger.info(
                        f"  Request Token: {request_token_clean[:20]}... (length: {len(request_token_clean)})"
                    )
                    logger.info(
                        f"  API Secret: {'*' * min(8, len(api_secret))}... (length: {len(api_secret)})"
                    )
                    ...
                    logger.info("Calling kite.generate_session with:")
                    logger.info(f"  Request token: {request_token_clean[:10]}...")
                    logger.info(f"  API key: {api_key[:10]}...")

                    data = kite.generate_session(
                        request_token=request_token_clean, api_secret=api_secret
                    )
                    logger.info(
                        f"generate_session succeeded. User: {data.get('user_id')}"
                    )
                    return data
                except Exception as e:
                    logger.error(
                        f"generate_session failed: {type(e).__name__}: {str(e)}"
                    )
                    logger.error(f"  API Key length: {len(api_key) if api_key else 0}")
                    logger.error(
                        f"  Request Token length: {len(request_token) if request_token else 0}"
                    )
                    logger.error(
                        f"  API Secret length: {len(api_secret) if api_secret else 0}"
                    )
                    logger.error("  ⚠️  IMPORTANT: If checksum error persists, verify:")
                    logger.error(
                        "     1. API secret in .env matches Zerodha Kite Connect app EXACTLY"
                    )
                    logger.error("     2. API key matches the one used in auth URL")
                    logger.error(
                        "     3. Request token is fresh (not expired - tokens expire after ~2 minutes)"
                    )
            ...
            expires_at = login_time + timedelta(
                hours=24
            )  # Zerodha tokens last 24 hours

            logger.info(
                f"Successfully exchanged request token for access token. User: {token_data.get('user_id')}"
            )
            ...
            try:
                ...
                if isinstance(e, KiteException):
                    logger.error(
                        f"KiteConnect API error: {e.message if hasattr(e, 'message') else error_msg}"
                    )
            ...
            if (
                ...
                or "checksum" in error_msg.lower()
            ):
                raise TradingError(
    ...
    async def _store_tokens(self, token_data: Dict[str, Any], state: str) -> None:
        ...
        try:
            ...
            if env_saved:
                logger.info(
                    f"Saved OAuth token to .env file for user: {token_data.get('user_id')}"
                )
            else:
                logger.warning(
                    "Failed to save token to .env file, saving to token file only"
                )
    ...
    async def _emit_oauth_event(self, event_type: str, data: Dict[str, Any]) -> None:
        ...
        try:
            event = Event(
                ...
                data={"event_type": event_type, "provider": "zerodha", **data},
            )
    ...
    async def logout(self) -> None:
        ...
        try:
            ...
            # Emit logout event
            await self._emit_oauth_event(
                "oauth_logout", {"timestamp": datetime.now(timezone.utc).isoformat()}
            )
