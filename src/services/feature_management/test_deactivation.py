...
from src.services.feature_management.models import (DependencyType,
                                                    FeatureConfig,
                                                    FeatureDependency,
                                                    FeatureMetadata,
                                                    FeatureType)
...
async def test_deactivation_system():
    ...
    try:
        ...
        success = await feature_service.enable_feature(
            test_feature.feature_id, reason="Testing deactivation system"
        )
        ...
        success = await feature_service.disable_feature(
            test_feature.feature_id, reason="Testing deactivation system", cascade=True
        )
        ...
        deactivation_status = await feature_service.get_deactivation_status(
        ...
        cleanup_success = await feature_service.force_cleanup_feature(
            test_feature.feature_id
        )
...
async def test_error_recovery():
    ...
    try:
        ...
        # Check error history
        error_history = await feature_service.get_error_history(
            problematic_feature.feature_id
        )
