...
from .models import FeatureConfig, FeatureStatus, FeatureType
...
class ServiceLifecycleManager(EventHandler):
    ...
    async def deactivate_feature(
        ...
        async with self.deactivation_lock:
            if feature_id in self.active_deactivations:
                logger.warning(
                    f"Deactivation already in progress for feature {feature_id}"
                )
            ...
            logger.info(
                f"Starting deactivation for feature {feature_id}: {reason or 'No reason provided'}"
            )
    ...
    async def _stage_preparation(
        self, operation: DeactivationOperation, feature_config: FeatureConfig
    ) -> None:
        ...
        try:
            ...
            snapshot = await self._create_resource_snapshot(
                operation.feature_id, feature_config
            )
            ...
            raise LifecycleManagerError(
                f"Preparation stage failed: {str(e)}", feature_id=operation.feature_id
            )
    ...
    async def _stage_stopping_tasks(
        ...
        try:
            ...
            raise LifecycleManagerError(
                f"Task stopping stage failed: {str(e)}", feature_id=operation.feature_id
            )
    ...
    async def _stage_cleanup_resources(
        ...
        logger.info(
            f"Stage 5 - Cleaning up resources for feature {operation.feature_id}"
        )
    ...
    async def _stage_finalization(
        ...
        try:
            ...
            await self._update_feature_state(
                operation.feature_id, FeatureStatus.DISABLED
            )
            ...
            raise LifecycleManagerError(
                f"Finalization stage failed: {str(e)}", feature_id=operation.feature_id
            )
    ...
    async def _identify_feature_resources(
        ...
        if feature_config.feature_type == FeatureType.AGENT:
            ...
            resources["tasks"].update(
                [TaskType.SYNC_ACCOUNT_BALANCES.value, TaskType.UPDATE_POSITIONS.value]
            )
        ...
        logger.info(
            f"Identified resources for feature {feature_id}: {self.feature_resources[feature_id]}"
        )
    ...
    async def _cleanup_memory_resources(self, feature_id: str) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Garbage collected {collected} objects for feature {feature_id}"
            )
    ...
    async def _cleanup_network_connections(self, feature_id: str) -> None:
        ...
        try:
            ...
            logger.error(
                f"Network connection cleanup failed for feature {feature_id}: {e}"
            )
    ...
    async def _update_feature_state(
    ...
    async def _log_deactivation_completion(
        self, operation: DeactivationOperation, reason: Optional[str]
    ) -> None:
    ...
    async def _handle_deactivation_failure(
        self, operation: DeactivationOperation, error_message: str
    ) -> None:
    ...
    async def _attempt_rollback(self, operation: DeactivationOperation) -> None:
        ...
        try:
            ...
            if not snapshot:
                logger.warning(
                    f"No resource snapshot available for rollback of {operation.feature_id}"
                )
                return
            ...
            logger.info(
                f"Successfully rolled back deactivation for feature {operation.feature_id}"
            )
    ...
    async def _emit_deactivation_event(
        self, feature_id: str, status: str, operation: DeactivationOperation
    ) -> None:
    ...
    async def _handle_system_error_event(self, event: Event) -> None:
        ...
        if error_data.get("severity") == "critical":
            logger.warning(
                "Critical system error detected, checking for emergency deactivation"
            )
    ...
    async def get_deactivation_status(
        self, feature_id: str
    ) -> Optional[DeactivationOperation]:
