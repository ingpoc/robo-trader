...
from .models import (
    FeatureConfig, FeatureState, FeatureDependency, DependencyType,
...
class DependencyResolver:
    ...
    def update_graph(self, features: Dict[str, FeatureConfig], states: Dict[str, FeatureState]) -> None:
    ...
    async def resolve_enable_order(
        ...
        try:
            ...
            logger.info(f"Resolved enable order for {len(features_to_enable)} features: {enable_order}")
    ...
    async def resolve_disable_order(
        ...
        try:
            ...
            logger.info(f"Resolved disable order for {len(features_to_disable)} features: {disable_order}")
    ...
    def _get_all_dependencies(self, feature_id: str, visited: Optional[Set[str]] = None) -> Set[str]:
    ...
    def _get_all_dependents(self, feature_id: str, visited: Optional[Set[str]] = None) -> Set[str]:
    ...
    def _topological_sort(self, feature_ids: Set[str]) -> List[str]:
        ...
        for feature_id in feature_ids:
            for dep in self.graph.get_dependencies(feature_id):
                if (dep.dependency_type == DependencyType.REQUIRES and 
                    not dep.optional and 
                    dep.feature_id in feature_ids):
