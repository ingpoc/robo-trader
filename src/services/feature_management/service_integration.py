...
class ServiceRegistry:
    ...
    async def get_services_by_feature(self, feature_id: str) -> List[ServiceInfo]:
        ...
        return [
            ...
            if service.feature_id == feature_id
        ]
...
class ServiceManagementIntegration:
    ...
    def __init__(
        ...
        self.feature_connections: Dict[str, Set[str]] = (
            {}
        )  # feature_id -> connection_ids
    ...
    def _initialize_service_mappings(self) -> None:
        ...
        service_mapping = {
            ...
            FeatureType.AGENT: {"claude_agent_service", "recommendation_service"},
            FeatureType.MONITOR: {"monitoring_service", "alert_service"},
    ...
    async def register_feature_services(
        self, feature_id: str, feature_config: FeatureConfig, service_ids: List[str]
    ) -> None:
        """
    ...
    async def register_feature_connection(
        self, feature_id: str, connection_id: str, connection_type: str, connection: Any
    ) -> None:
        ...
        logger.debug(
            f"Registered {connection_type} connection {connection_id} for feature {feature_id}"
        )
    ...
    async def deactivate_feature_services(
        ...
        if feature_id in self.active_operations:
            logger.warning(
                f"Service deactivation already in progress for feature {feature_id}"
            )
            return self.active_operations[feature_id]

        result = ServiceDeactivationResult(
            feature_id=feature_id, status=ServiceIntegrationStatus.IDLE
        )
        ...
        try:
            ...
            logger.info(
                f"Successfully completed service deactivation for feature {feature_id}"
            )
            ...
            error_msg = (
                f"Service deactivation failed for feature {feature_id}: {str(e)}"
            )
            result.errors.append(error_msg)
            result.status = ServiceIntegrationStatus.FAILED
            logger.error(error_msg)
    ...
    async def _stop_feature_services(
        ...
        for service in services:
            try:
                ...
                if success:
                    ...
                    logger.debug(
                        f"Stopped service {service.service_id} for feature {feature_id}"
                    )
        ...
        logger.info(
            f"Stopped {len(stopped_services)} services for feature {feature_id}"
        )
    ...
    async def _close_feature_connections(
        ...
        logger.info(
            f"Closed {len(closed_connections)} connections for feature {feature_id}"
        )
    ...
    async def _close_feature_websockets(
        ...
        # Update result
        if feature_id in self.active_operations:
            self.active_operations[feature_id].websockets_closed.extend(
                closed_websockets
            )

        logger.info(
            f"Closed {len(closed_websockets)} WebSockets for feature {feature_id}"
        )
    ...
    async def _clean_feature_cache(
        ...
        # Update result
        if feature_id in self.active_operations:
            self.active_operations[feature_id].cache_entries_cleared.extend(
                cleared_entries
            )

        logger.info(
            f"Cleared {len(cleared_entries)} cache entries for feature {feature_id}"
        )
    ...
    async def _create_service_state_snapshot(
        ...
        try:
            ...
            connections = self.feature_connections.get(feature_id, set())
            for connection_id in connections:
                connection_state = {"connection_id": connection_id, "status": "active"}
            ...
            websockets = self.feature_websockets.get(feature_id, set())
            for websocket_id in websockets:
                websocket_state = {"websocket_id": websocket_id, "status": "connected"}
                snapshot.websocket_states[websocket_id] = websocket_state

        except Exception as e:
            logger.error(
                f"Failed to create service state snapshot for feature {feature_id}: {e}"
            )
    ...
    async def get_feature_connection_info(
        self, feature_id: str
    ) -> List[ConnectionInfo]:
    ...
    async def rollback_service_deactivation(self, feature_id: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Successfully rolled back service deactivation for feature {feature_id}"
            )
            return True
    ...
    async def _emit_service_event(
        self, feature_id: str, event_type: str, result: ServiceDeactivationResult
    ) -> None:
    ...
    async def get_integration_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tracked_services": sum(
                len(services) for services in self.feature_services.values()
            ),
            "tracked_connections": sum(
                len(connections) for connections in self.feature_connections.values()
            ),
            "tracked_websockets": sum(
                len(websockets) for websockets in self.feature_websockets.values()
            ),
            "tracked_cache_keys": sum(
                len(keys) for keys in self.feature_cache_keys.values()
            ),
    ...
    async def close_all_connections(self) -> None:
        ...
        for connection_id, connection in self.database_connections.items():
            try:
                ...
                logger.error(
                    f"Failed to close database connection {connection_id}: {e}"
                )
    ...
    async def close(self) -> None:
        ...
        for feature_id, result in self.active_operations.items():
            logger.warning(
                f"Cancelling active service operation for feature {feature_id}"
            )
            result.status = ServiceIntegrationStatus.FAILED
