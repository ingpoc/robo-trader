...
from .models import FeatureConfig, FeatureState, FeatureStatus
...
class EventBroadcastingService:
    ...
    async def subscribe_to_events(
        ...
        Returns:
            Subscription ID
        """
        subscription_id = f"sub_{int(datetime.now(timezone.utc).timestamp() * 1000)}_{subscriber_id}"
        ...
        logger.info(f"Created subscription {subscription_id} for subscriber {subscriber_id}")
    ...
    async def broadcast_event(
        ...
        logger.debug(f"Broadcasted event {event.event_type.value} for feature {feature_id}")
    ...
    async def broadcast_progress_update(
        ...
        logger.debug(f"Broadcasted progress update for operation {operation_id}: {progress_percentage}% - {message}")
    ...
    async def _broadcast_to_subscribers(self, event: BroadcastEvent) -> None:
        ...
        for subscription in matching_subscriptions:
            try:
                ...
                logger.error(f"Failed to send event to subscription {subscription.subscription_id}: {str(e)}")

    async def _call_callback_safely(self, callback: callable, event: BroadcastEvent) -> None:
    ...
    async def _send_to_websocket_safely(self, websocket: Any, event: BroadcastEvent) -> None:
    ...
    async def _forward_to_event_bus(self, event: BroadcastEvent) -> None:
        ...
        try:
            ...
            bus_event_type = event_type_mapping.get(event.event_type, EventType.SYSTEM_HEALTH_CHECK)
