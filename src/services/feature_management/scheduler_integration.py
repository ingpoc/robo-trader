...
from src.models.scheduler import QueueName, TaskStatus, TaskType
...
class BackgroundSchedulerIntegration:
    ...
    def __init__(
        ...
        self.task_type_features: Dict[TaskType, Set[str]] = (
            {}
        )  # task_type -> feature_ids
    ...
    async def register_feature_tasks(
        ...
        logger.info(
            f"Registered {len(task_types)} tasks and {len(queues) or 0} queues for feature {feature_id}"
        )
    ...
    async def deactivate_feature_scheduler_resources(
        ...
        if feature_id in self.active_operations:
            logger.warning(
                f"Scheduler deactivation already in progress for feature {feature_id}"
            )
            return self.active_operations[feature_id]

        result = SchedulerDeactivationResult(
            feature_id=feature_id, status=SchedulerIntegrationStatus.IDLE
        )
        ...
        try:
            ...
            logger.info(
                f"Successfully completed scheduler deactivation for feature {feature_id}"
            )
            ...
            error_msg = (
                f"Scheduler deactivation failed for feature {feature_id}: {str(e)}"
            )
            result.errors.append(error_msg)
            result.status = SchedulerIntegrationStatus.FAILED
            logger.error(error_msg)
    ...
    async def _stop_feature_tasks(
        ...
        for task_id in task_ids:
            try:
                ...
                logger.error(
                    f"Failed to stop task {task_id} for feature {feature_id}: {e}"
                )
    ...
    async def _cancel_feature_queues(
        ...
        for queue_name in queues:
            try:
                ...
                logger.debug(
                    f"Cancelled queue {queue_name.value} for feature {feature_id}"
                )

            except Exception as e:
                logger.error(
                    f"Failed to cancel queue {queue_name.value} for feature {feature_id}: {e}"
                )
        ...
        logger.info(
            f"Cancelled {len(cancelled_queues)} queues for feature {feature_id}"
        )
    ...
    async def _cleanup_feature_schedules(
        ...
        if not self.background_scheduler:
            logger.warning(
                "Background scheduler not available, cannot clean up schedules"
            )
            return
        ...
        for schedule_id in schedules:
            try:
                ...
                logger.error(
                    f"Failed to remove schedule {schedule_id} for feature {feature_id}: {e}"
                )

        # Get result and update
        if feature_id in self.active_operations:
            self.active_operations[feature_id].schedules_removed.extend(
                removed_schedules
            )

        logger.info(
            f"Removed {len(removed_schedules)} schedules for feature {feature_id}"
        )
        ...
        for queue_name in queues:
            try:
                ...
                orchestration_status = (
                    await self.queue_orchestration_layer.get_orchestration_status()
                )
    ...
    async def discover_feature_tasks(
        ...
        for task_type in expected_task_types:
            try:
                ...
                logger.error(
                    f"Failed to discover tasks for type {task_type.value}: {e}"
                )
        ...
        if discovered_tasks:
            ...
            logger.info(
                f"Discovered {len(discovered_tasks)} tasks for feature {feature_id}"
            )
    ...
    async def cleanup_completed_tasks(
        self, feature_id: str, days_to_keep: int = 7
    ) -> int:
        ...
        try:
            ...
            logger.info(
                f"Cleaned up {deleted_count} old tasks for feature {feature_id}"
            )
    ...
    async def rollback_scheduler_deactivation(self, feature_id: str) -> bool:
        ...
        try:
            ...
            for queue_name in queues:
                try:
                    ...
                    if self.queue_orchestration_layer:
                        ...
                        logger.debug(
                            f"Restarted queue {queue_name.value} during rollback"
                        )
            ...
            logger.info(
                f"Successfully rolled back scheduler deactivation for feature {feature_id}"
            )
            return True
    ...
    async def _emit_scheduler_event(
        self, feature_id: str, event_type: str, result: SchedulerDeactivationResult
    ) -> None:
    ...
    async def get_integration_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tracked_queues": sum(
                len(queues) for queues in self.feature_queues.values()
            ),
            "tracked_schedules": sum(
                len(schedules) for schedules in self.feature_schedules.values()
            ),
            "services_available": {
    ...
    async def close(self) -> None:
        ...
        for feature_id, result in self.active_operations.items():
            logger.warning(
                f"Cancelling active scheduler operation for feature {feature_id}"
            )
            result.status = SchedulerIntegrationStatus.FAILED
