...
from ...core.errors import ErrorCategory, ErrorSeverity
...
class ErrorRecoveryManager:
    ...
    async def handle_error(
        ...
        error_info = self._create_error_info(
            feature_id, operation, stage, error, context
        )
        ...
        logger.error(
            f"Error in feature {feature_id} during {operation} at stage {stage}: {str(error)}"
        )
    ...
    def _categorize_error(self, error: Exception) -> ErrorCategory:
        ...
        if any(
            ...
            keyword in error_message
            for keyword in ["database", "sql", "connection pool"]
        ):
            return ErrorCategory.DATABASE_ERROR
        elif any(
            keyword in error_message
            for keyword in ["service", "microservice", "endpoint"]
        ):
            ...
            keyword in error_message
            for keyword in ["memory", "disk", "resource", "file"]
        ):
            return ErrorCategory.RESOURCE_ERROR
        elif any(
            keyword in error_message
            for keyword in ["permission", "access", "unauthorized", "forbidden"]
        ):
    ...
    async def _start_recovery_process(self, session: ErrorRecoverySession) -> None:
        ...
        try:
            ...
            if not actions:
                logger.warning(
                    f"No recovery actions available for error category {session.primary_error.category}"
                )
            ...
            for action in actions:
                try:
                    ...
                    if result.success:
                        ...
                        logger.info(
                            f"Successfully recovered from error in session {session.session_id}"
                        )
                        break
                    else:
                        logger.warning(
                            f"Recovery action {action.action_id} failed: {result.message}"
                        )

                except Exception as e:
                    logger.error(
                        f"Exception executing recovery action {action.action_id}: {str(e)}"
                    )
                    session.recovery_results.append(
                        RecoveryResult(
            ...
            if session.status == "active":
                ...
                logger.error(
                    f"Failed to recover from error in session {session.session_id}"
                )

        except Exception as e:
            logger.error(
                f"Recovery process failed for session {session.session_id}: {str(e)}"
            )
            session.status = "failed"
    ...
    async def _execute_recovery_action(
        self, session: ErrorRecoverySession, action: RecoveryAction
    ) -> RecoveryResult:
        ...
        try:
            ...
            result_message = await asyncio.wait_for(
                action.handler(session, action), timeout=action.timeout_seconds
            )
    ...
    async def _retry_with_backoff(
        ...
        for attempt in range(action.max_attempts):
            try:
                ...
                logger.warning(
                    f"Retry attempt {attempt + 1} failed, retrying in {delay:.2f}s: {str(e)}"
                )
    ...
    async def _rollback_database_transaction(
        self, session: ErrorRecoverySession, action: RecoveryAction
    ) -> str:
    ...
    async def _restart_service_and_retry(
        self, session: ErrorRecoverySession, action: RecoveryAction
    ) -> str:
    ...
    async def _cleanup_resources_and_retry(
        self, session: ErrorRecoverySession, action: RecoveryAction
    ) -> str:
    ...
    async def _rollback_configuration(
        self, session: ErrorRecoverySession, action: RecoveryAction
    ) -> str:
    ...
    async def _emit_error_event(
        self, error_info: ErrorInfo, session: ErrorRecoverySession
    ) -> None:
        """Emit an error event."""
    ...
    async def _emit_recovery_event(self, session: ErrorRecoverySession) -> None:
        ...
        await self.event_bus.publish(
            Event(
                ...
                timestamp=datetime.now(timezone.utc).isoformat(),
                source="error_recovery",
                data={"session": session.to_dict(), "feature_id": session.feature_id},
            )
        )

    async def get_recovery_session(
        self, session_id: str
    ) -> Optional[ErrorRecoverySession]:
    ...
    async def add_custom_recovery_strategy(
        self, category: ErrorCategory, action: RecoveryAction
    ) -> None:
        ...
        logger.info(
            f"Added custom recovery strategy {action.action_id} for category {category.value}"
        )
    ...
    async def close(self) -> None:
        ...
        for session_id, session in list(self.active_sessions.items()):
            ...
            session.completed_at = datetime.now(timezone.utc).isoformat()
            logger.warning(
                f"Closed active recovery session {session_id} due to shutdown"
            )
