...
class ResourceTracker:
    ...
    def track_resource(self, feature_id: str, resource_type: str, resource_id: str, resource: Any) -> None:
        ...
        if resource_type == "memory_object":
            self.feature_resources[feature_id]["memory_objects"][resource_id] = id(resource)
            ...
            weak_ref = weakref.ref(resource, lambda ref: self._cleanup_weak_ref(resource_id))
...
class ResourceCleanupManager:
    ...
    async def track_feature_resource(
        ...
        """Track a resource for a feature."""
        self.resource_tracker.track_resource(feature_id, resource_type, resource_id, resource)
    ...
    async def cleanup_feature_resources(
        ...
        if feature_id in self.active_operations:
            logger.warning(f"Resource cleanup already in progress for feature {feature_id}")
            return self.active_operations[feature_id]
        
        result = ResourceCleanupResult(
            feature_id=feature_id,
            status=ResourceCleanupStatus.IDLE
        )
        ...
        try:
            ...
            result.memory_freed_mb = result.metrics_before.memory_usage_mb - result.metrics_after.memory_usage_mb
            
            # Mark as completed
            result.status = ResourceCleanupStatus.COMPLETED
            result.completed_at = datetime.now(timezone.utc).isoformat()
            
            logger.info(f"Successfully completed resource cleanup for feature {feature_id}")
            logger.info(f"Freed {result.memory_freed_mb:.2f} MB memory, deleted {result.files_deleted} files")
    ...
    async def _cleanup_memory_resources(
        ...
        try:
            ...
            if self.cleanup_policies["force_memory_cleanup"]:
                ...
                logger.debug(f"Garbage collected {collected_objects} objects for feature {feature_id}")
        ...
        try:
            ...
            if feature_temp_dir.exists():
                try:
                    ...
                    warning_msg = f"Failed to remove temp directory {feature_temp_dir}: {str(e)}"
                    result.warnings.append(warning_msg)
                    logger.warning(warning_msg)
            ...
            for temp_dir in temp_directories:
                try:
                    ...
                    warning_msg = f"Failed to remove temp directory {temp_dir}: {str(e)}"
                    result.warnings.append(warning_msg)
                    logger.warning(warning_msg)
            ...
            error_msg = f"Temporary data cleanup failed for feature {feature_id}: {str(e)}"
            result.errors.append(error_msg)
            logger.error(error_msg)
    ...
    async def _get_resource_metrics(self) -> ResourceMetrics:
        ...
        try:
            ...
            # Temporary files
            temp_files_count = len(list(self.temp_dir.rglob("*"))) if self.temp_dir.exists() else 0
    ...
    async def _create_resource_snapshot(self, feature_id: str) -> ResourceSnapshot:
        ...
        try:
            ...
            logger.error(f"Failed to create resource snapshot for feature {feature_id}: {e}")
    ...
    async def get_cleanup_status(self, feature_id: str) -> Optional[ResourceCleanupResult]:
    ...
    async def close(self) -> None:
        ...
        for feature_id, result in self.active_operations.items():
            logger.warning(f"Cancelling active cleanup operation for feature {feature_id}")
            result.status = ResourceCleanupStatus.FAILED
