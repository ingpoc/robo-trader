import { useState, useEffect, useRef } from 'react'
import { wsClient } from '@/api/websocket'
import { useAgents } from './useAgents'

export type ClaudeStatus = 'unavailable' | 'idle' | 'analyzing' | 'authenticated' | 'connected/idle'

export interface ClaudeStatusInfo {
  status: ClaudeStatus
  message: string
  currentTask?: string
}

/**
 * Hook to track Claude Paper Trader agent status
 * - unavailable: Claude agent is not running or disconnected
 * - idle: Claude agent is connected but not actively analyzing
 * - analyzing: Claude agent is actively running analysis
 */
export function useClaudeStatus(): ClaudeStatusInfo {
  const [statusInfo, setStatusInfo] = useState<ClaudeStatusInfo>({
    status: 'unavailable',
    message: 'Initializing...',
  })

  // Track last activity timestamp
  const lastActivityRef = useRef<number>(Date.now())

  useEffect(() => {
    // Subscribe to WebSocket for real-time Claude status updates
    const unsubscribe = wsClient.subscribe((message: any) => {
      // Check for Claude SDK status updates from backend
      if (message.type === 'claude_status_update') {
        const sdkStatus = message.status
        if (sdkStatus === 'connected/idle') {
          setStatusInfo({
            status: 'connected/idle',
            message: 'SDK Connected - Ready for queries',
          })
        } else if (sdkStatus === 'authenticated') {
          setStatusInfo({
            status: 'authenticated',
            message: 'CLI Authenticated - SDK not connected',
          })
        } else if (sdkStatus === 'disconnected') {
          setStatusInfo({
            status: 'unavailable',
            message: 'SDK Disconnected',
          })
        }
        return
      }

      // Check for system health updates (includes Claude agent status)
      if (message.type === 'system_health_update') {
        const components = message.components || {}
        const claudeAgent = components.claudeAgent || {}

        if (claudeAgent.status === 'active') {
          setStatusInfo({
            status: 'connected/idle',
            message: 'SDK Connected - Active',
          })
        } else if (claudeAgent.status === 'authenticated') {
          setStatusInfo({
            status: 'authenticated',
            message: 'CLI Authenticated - SDK not connected',
          })
        } else if (claudeAgent.status === 'inactive') {
          setStatusInfo({
            status: 'unavailable',
            message: 'SDK Inactive',
          })
        }
        return
      }

      // Check for agent-specific status updates (legacy)
      if (message.type === 'agent_status_update' && message.agent_name === 'claude_paper_trader') {
        lastActivityRef.current = Date.now()

        const agentStatus = message.status
        if (agentStatus === 'running') {
          setStatusInfo({
            status: 'analyzing',
            message: message.message || 'Analyzing market data...',
            currentTask: message.current_task,
          })
        } else if (agentStatus === 'idle') {
          setStatusInfo({
            status: 'idle',
            message: 'Ready',
          })
        } else if (agentStatus === 'error') {
          setStatusInfo({
            status: 'unavailable',
            message: 'Error',
          })
        }
      }

      // Check for AI task updates from dashboard data
      if (message.type === 'dashboard_update' && message.data?.ai_status) {
        const aiStatus = message.data.ai_status
        if (aiStatus.current_task && aiStatus.current_task !== 'Idle') {
          lastActivityRef.current = Date.now()
          setStatusInfo({
            status: 'analyzing',
            message: aiStatus.current_task,
            currentTask: aiStatus.current_task,
          })
        } else {
          // Check if recently active (within last 5 seconds)
          const timeSinceActivity = Date.now() - lastActivityRef.current
          if (timeSinceActivity > 5000) {
            setStatusInfo({
              status: 'idle',
              message: 'Ready',
            })
          }
        }
      }
    })

    return () => {
      unsubscribe()
    }
  }, [])


  return statusInfo
}
